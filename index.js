...
app.post('/api/POST/accept-health', (req, res) => {
    ...
    connection.query(`
        INSERT INTO tblhealthhistory (CertiID, fname, lname, street, date, time, purpose, DateAccept)
        SELECT CertiID, fname, lname, street, date, time, purpose, ? FROM tblresihealth WHERE CertiID = ?
    `, [dateAccept, CertiID], (err) => {
        if (err) {
            console.error('Error accepting barangay appointment:', err)  
            return res.status(500).json({ success: false, message: 'Failed to accept appointment' })  
...
//this is for GET the inventory of the barangay hall in certificates
app.get('/api/GET/inventory-barangay', (req,res) => {
    connection.query('SELECT * FROM barangayinventory', (err, rows) =>{
        if(err){
            console.error('Error fetching barangay certificates inventory:', error)
...
//this is for GET the inventory of the barangay health center
app.get('/api/GET/inventory-health-center', (req, res) => {
    connection.query('SELECT * FROM healthcenterinventory', (err, rows) => {
        if(err){
            console.error('Error fetching Health Center Inventory:', error)
            return res.status(500).json({success: false, message: 'Failed to fetch Inventory'})
        }
        res.json(rows)
    })
